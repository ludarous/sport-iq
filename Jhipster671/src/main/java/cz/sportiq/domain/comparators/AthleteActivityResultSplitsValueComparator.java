...
public class AthleteActivityResultSplitsValueComparator implements Comparator<AthleteActivityResult> {
    ...
    public int compare(AthleteActivityResult o1, AthleteActivityResult o2) {
        ...
        List<Float> splitValues1 = o1.getAthleteActivityResultSplits().stream().map(rs -> rs.getValue()).collect(Collectors.toList());
        List<Float> splitValues2 = o2.getAthleteActivityResultSplits().stream().map(rs -> rs.getValue()).collect(Collectors.toList());
