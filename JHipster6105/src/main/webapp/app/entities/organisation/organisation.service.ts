...
export class OrganisationService {
  ...
  update(organisation: IOrganisation): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(organisation);
    return this.http
      .put<IOrganisation>(this.resourceUrl, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  protected convertDateFromClient(organisation: IOrganisation): IOrganisation {
    const copy: IOrganisation = Object.assign({}, organisation, {
      created: organisation.created && organisation.created.isValid() ? organisation.created.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((organisation: IOrganisation) => {
        organisation.created = organisation.created ? moment(organisation.created) : undefined;
      });
    }
    return res;
  }
