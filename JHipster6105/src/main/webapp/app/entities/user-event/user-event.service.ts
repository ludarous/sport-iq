...
export class UserEventService {
  public resourceUrl = SERVER_API_URL + 'api/user-events';
  ...
  protected convertDateFromClient(userEvent: IUserEvent): IUserEvent {
    const copy: IUserEvent = Object.assign({}, userEvent, {
      registrationDate:
        userEvent.registrationDate && userEvent.registrationDate.isValid() ? userEvent.registrationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((userEvent: IUserEvent) => {
        userEvent.registrationDate = userEvent.registrationDate ? moment(userEvent.registrationDate) : undefined;
      });
    }
    return res;
  }
