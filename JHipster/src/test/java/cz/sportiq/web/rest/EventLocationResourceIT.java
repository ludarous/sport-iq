...
public class EventLocationResourceIT {
     ...
     *
     * @see cz.sportiq.repository.search.EventLocationSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createEventLocationWithExistingId() throws Exception {
        ...
        // Validate the EventLocation in Elasticsearch
        verify(mockEventLocationSearchRepository, times(0)).save(eventLocation);
    }
    ...
    public void deleteEventLocation() throws Exception {
        ...
        // Validate the EventLocation in Elasticsearch
        verify(mockEventLocationSearchRepository, times(1)).deleteById(eventLocation.getId());
    }

    @Test
    @Transactional
    public void searchEventLocation() throws Exception {
        // Initialize the database
        eventLocationRepository.saveAndFlush(eventLocation);
        when(mockEventLocationSearchRepository.search(queryStringQuery("id:" + eventLocation.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(eventLocation), PageRequest.of(0, 1), 1));
        // Search the eventLocation
        restEventLocationMockMvc.perform(get("/api/_search/event-locations?query=id:" + eventLocation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(eventLocation.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
