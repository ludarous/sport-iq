...
export class AthleteActivityResultResolve implements Resolve<IAthleteActivityResult> {
  ...
  resolve(route: ActivatedRouteSnapshot): Observable<IAthleteActivityResult> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(map((athleteActivityResult: HttpResponse<AthleteActivityResult>) => athleteActivityResult.body));
    }
