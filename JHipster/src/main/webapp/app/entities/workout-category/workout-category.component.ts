...
export class WorkoutCategoryComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.workoutCategoryService
                ...
                .subscribe(
                    (res: HttpResponse<IWorkoutCategory[]>) => this.paginateWorkoutCategories(res.body, res.headers),
    ...
    registerChangeInWorkoutCategories() {
        this.eventSubscriber = this.eventManager.subscribe('workoutCategoryListModification', response => this.loadAll());
    }
    ...
    private paginateWorkoutCategories(data: IWorkoutCategory[], headers: HttpHeaders) {
