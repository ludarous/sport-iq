...
export class AthleteEventComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.athleteEventService
        ...
        .subscribe((res: HttpResponse<IAthleteEvent[]>) => this.paginateAthleteEvents(res.body, res.headers));
      return;
    }
    this.athleteEventService
      .query({
  ...
  trackId(index: number, item: IAthleteEvent) {
    return item.id;
  }

  registerChangeInAthleteEvents() {
    this.eventSubscriber = this.eventManager.subscribe('athleteEventListModification', () => this.loadAll());
  }

  delete(athleteEvent: IAthleteEvent) {
  ...
  protected paginateAthleteEvents(data: IAthleteEvent[], headers: HttpHeaders) {
