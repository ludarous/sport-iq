...
export class EventLocationComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.eventLocationService
        ...
        .subscribe((res: HttpResponse<IEventLocation[]>) => this.paginateEventLocations(res.body, res.headers));
      return;
    }
    this.eventLocationService
      .query({
  ...
  trackId(index: number, item: IEventLocation) {
    return item.id;
  }

  registerChangeInEventLocations() {
    this.eventSubscriber = this.eventManager.subscribe('eventLocationListModification', () => this.loadAll());
  }

  delete(eventLocation: IEventLocation) {
  ...
  protected paginateEventLocations(data: IEventLocation[], headers: HttpHeaders) {
