...
export class ActivityCategoryComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.activityCategoryService
        ...
        .subscribe((res: HttpResponse<IActivityCategory[]>) => this.paginateActivityCategories(res.body, res.headers));
      return;
    }
    this.activityCategoryService
      .query({
  ...
  trackId(index: number, item: IActivityCategory) {
    return item.id;
  }

  registerChangeInActivityCategories() {
    this.eventSubscriber = this.eventManager.subscribe('activityCategoryListModification', () => this.loadAll());
  }

  delete(activityCategory: IActivityCategory) {
  ...
  protected paginateActivityCategories(data: IActivityCategory[], headers: HttpHeaders) {
