...
public class UserActivityResultResourceIT {
    ...
    @Autowired
    private UserActivityResultService userActivityResultService;
    ...
    public void createUserActivityResult() throws Exception {
        ...
        UserActivityResultDTO userActivityResultDTO = userActivityResultMapper.toDto(userActivityResult);
        restUserActivityResultMockMvc.perform(post("/api/user-activity-results").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(userActivityResultDTO)))
    ...
    public void updateUserActivityResult() throws Exception {
        ...
        updatedUserActivityResult
            .value(UPDATED_VALUE)
            .compareValue(UPDATED_COMPARE_VALUE);
        UserActivityResultDTO userActivityResultDTO = userActivityResultMapper.toDto(updatedUserActivityResult);

        restUserActivityResultMockMvc.perform(put("/api/user-activity-results").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(userActivityResultDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteUserActivityResult() throws Exception {
        ...
        restUserActivityResultMockMvc.perform(delete("/api/user-activity-results/{id}", userActivityResult.getId()).with(csrf())
