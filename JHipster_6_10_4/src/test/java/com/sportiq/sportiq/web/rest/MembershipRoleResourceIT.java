...
public class MembershipRoleResourceIT {
    ...
    public void createMembershipRole() throws Exception {
        ...
        MembershipRoleDTO membershipRoleDTO = membershipRoleMapper.toDto(membershipRole);
        restMembershipRoleMockMvc.perform(post("/api/membership-roles").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(membershipRoleDTO)))
    ...
    public void updateMembershipRole() throws Exception {
        ...
        updatedMembershipRole
            .name(UPDATED_NAME);
        MembershipRoleDTO membershipRoleDTO = membershipRoleMapper.toDto(updatedMembershipRole);

        restMembershipRoleMockMvc.perform(put("/api/membership-roles").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(membershipRoleDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteMembershipRole() throws Exception {
        ...
        restMembershipRoleMockMvc.perform(delete("/api/membership-roles/{id}", membershipRole.getId()).with(csrf())
