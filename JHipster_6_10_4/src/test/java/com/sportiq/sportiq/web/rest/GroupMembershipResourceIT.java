...
public class GroupMembershipResourceIT {
    ...
    @Mock
    private GroupMembershipService groupMembershipServiceMock;
    ...
    public void createGroupMembership() throws Exception {
        ...
        GroupMembershipDTO groupMembershipDTO = groupMembershipMapper.toDto(groupMembership);
        restGroupMembershipMockMvc.perform(post("/api/group-memberships").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(groupMembershipDTO)))
    ...
    public void getAllGroupMembershipsWithEagerRelationshipsIsEnabled() throws Exception {
        when(groupMembershipServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restGroupMembershipMockMvc.perform(get("/api/group-memberships?eagerload=true"))
            .andExpect(status().isOk());

        verify(groupMembershipServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    public void updateGroupMembership() throws Exception {
        ...
        updatedGroupMembership
            .created(UPDATED_CREATED);
        GroupMembershipDTO groupMembershipDTO = groupMembershipMapper.toDto(updatedGroupMembership);

        restGroupMembershipMockMvc.perform(put("/api/group-memberships").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(groupMembershipDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteGroupMembership() throws Exception {
        ...
        restGroupMembershipMockMvc.perform(delete("/api/group-memberships/{id}", groupMembership.getId()).with(csrf())
