...
public class OrganisationMembershipResourceIT {
    ...
    @Mock
    private OrganisationMembershipService organisationMembershipServiceMock;
    ...
    public void createOrganisationMembership() throws Exception {
        ...
        OrganisationMembershipDTO organisationMembershipDTO = organisationMembershipMapper.toDto(organisationMembership);
        restOrganisationMembershipMockMvc.perform(post("/api/organisation-memberships").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(organisationMembershipDTO)))
    ...
    public void getAllOrganisationMembershipsWithEagerRelationshipsIsEnabled() throws Exception {
        when(organisationMembershipServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restOrganisationMembershipMockMvc.perform(get("/api/organisation-memberships?eagerload=true"))
            .andExpect(status().isOk());

        verify(organisationMembershipServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    public void updateOrganisationMembership() throws Exception {
        ...
        updatedOrganisationMembership
            .created(UPDATED_CREATED);
        OrganisationMembershipDTO organisationMembershipDTO = organisationMembershipMapper.toDto(updatedOrganisationMembership);

        restOrganisationMembershipMockMvc.perform(put("/api/organisation-memberships").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
            .content(TestUtil.convertObjectToJsonBytes(organisationMembershipDTO)))
            .andExpect(status().isOk());
    ...
    public void deleteOrganisationMembership() throws Exception {
        ...
        restOrganisationMembershipMockMvc.perform(delete("/api/organisation-memberships/{id}", organisationMembership.getId()).with(csrf())
